    ...
    int byte_rcv = 0;
        while(game){
            if(who == 1 || game == 1){
                ...
                do{
                    byte_rcv = my_read(sock_fd, (void *) &mv, sizeof(mv), 0, srv_sockaddr, &srv_sockaddr_size);
            ...
            if(who == 0 || game == 1){
                ...
                do{
                    ...
                    if(ev1.data.fd == sock_fd){
                        struct ping pm;
                        my_read(sock_fd, (void*) &pm, sizeof(pm), 0, srv_sockaddr, &srv_sockaddr_size);
                        if(pm.v == PING){
                            struct move mvv;
                            mvv.flags = PONG;
                            my_write(sock_fd, (const void*) &mvv, sizeof(mvv), 0, srv_sockaddr, srv_sockaddr_size);                              
...
int main(int argc, char ** argv){
    ...
    if(!strcmp(argv[2], "unix")){
        ...
        my_read(sock_fd,(void *) &msg, sizeof(msg), 0, srv_sockaddr, &srv_sockaddr_size);
        if(msg.v == HELLO){
            my_id = msg.who;
        }
        ...
        my_read(sock_fd,(void *) &lmsg, sizeof(lmsg), 0, srv_sockaddr, &srv_sockaddr_size);
